...
- **`FlipkartDealsProvider`** (`providers/flipkart.provider.ts`) is the first real provider and is a deliberate stub: `isConfigured()` genuinely checks `FLIPKART_AFFILIATE_ID`/`FLIPKART_AFFILIATE_TOKEN`, but `search()` always returns `'unavailable'` — no network call is made, because Flipkart's actual affiliate/API request-response contract hasn't been verified against real docs yet. Do not guess that contract; implement it only once real credentials + verified API docs are in hand (see the file's doc comment for the checklist).
...
- **`DealsProviderRegistry`** (`providers/deals-provider.registry.ts`) holds every registered real provider via the `DEALS_PROVIDERS` multi-provider injection token (wired in `deals.module.ts`). Add a new marketplace (Amazon, Myntra, Croma, Ajio, Tata CLiQ) by implementing `DealsProvider` and appending it to that token's factory — `DealsService` and the controller never need to change.
...
- **`DealClick`** (`schemas/deal-click.schema.ts`, `userId`-scoped, append-only) — records a click-through via `POST /deals/offers/:offerId/click`, and what URL it actually resolved to (`isAffiliateResolved` is `false` for every provider today — see below).
...
**Affiliate links**: `MerchantOffer.affiliateUrl` is never synthesized — `MarketplaceDealsService.recordClick()` falls back to the plain `dealUrl` and stamps `isAffiliateResolved: false` until a provider implements a real, verified affiliate-link resolver (Flipkart's isn't implemented yet — see `providers/flipkart.provider.ts`).
